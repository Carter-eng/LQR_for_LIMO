...
LQR code adapted by Carter Berlind to fit the Bicyle dynamical model (for AgileX Limo)
For more info on dynamic model, see: https://dingyan89.medium.com/simple-understanding-of-kinematic-bicycle-model-81cac6420357
...
class LQR:
    def __init__(
        ...
        :param L: wheelbase in meters, default to 0.2m from 
    ...
    def getAB(
        ...
        :return: A matrix ---> 4x4 NumPy array (models how system responds to a given state)
    ...
    def getK(
        ...
        """
        This function retrieves a gain matrix K, allowing us to find our control based on our state.
        ...
        """
        N =timeSteps
        # Create a list of N + 1 elements, P is time varying cost matrix based on both state and control effort
        P = [None] * (N + 1)
        # At the final time we don't have a control input so P is just our state cost
        P[N] = self.Q

        for i in range(N, 0, -1):
            # Discrete-time Algebraic Riccati Equation (ARE) to calculate the optimal
            # P matrix. This is a dynamic programming method, meaning we start 
            # from the final time and work backward to find our desired matrix
    ...
    def getControl(
        ...
        u_star = K@x_error
        # Create variable to check for clipping
        u_unclipped = u_star
        ...
        # Tell the user if inputs were clipped
    ...
    def lqr(
        ...
        :param xd: The desired state of the system
            4x1 NumPy Array given the state is [x,y,theta,v] --->
            [meters, meters, radians, m/s]
        :param time: The number of timesteps to stabilize -> int
        :param A: A matrix in linear model x_t = A @ x_prev + B @ u_prev -> 
            4x4 NumPy array
        :param B: B matrix in linear model x_t = A @ x_prev + B @ u_prev -> 
            4x2 NumPy array
        ...
        """
        # Set error based on the current and desired state
        x_error = xd-x
